...
export default function ProductTabs({ activeId, onActiveIdChange, tabLists }) {
  const tab = tabLists.map((tabList) => {
    ...
    return (
      <li
        ...
        className={
          isSelected
            ? "tab-list-tab-item tab-list-tab-item-active"
            : "tab-list-tab-item"
        }
        onClick={() => onActiveIdChange(tabList.id)}
      >
        {tabList.name}
      </li>
    );
  ...
  return (
    <div className="tab-list">
      ...
      <TabPanel tabList={tabLists[activeId]} />
    </div>
  );
}
  ...
  const tabLabels = tabList.labels.map((label) => {
    return (
      <div key={label.name} className="tab-panel-label">
        <div className="tab-panel-label-icon">{label.icon}</div>
  ...
  return (
    <div className="tab-panel">
      <div className="tab-panel-img-content">
        <img
          className="tab-panel-img"
          src={tabList.image.src}
          alt={tabList.image.alt}
        />
      </div>
      <div className="tab-panel-text-content">
        ...
        <div className="tab-panel-labels">{tabLabels}</div>
      </div>
    </div>
  );
}
